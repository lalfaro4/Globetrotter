...
CREATE TABLE IF NOT EXISTS `GlobetrotterV1`.`activity` (
  ...
  `start_location` VARCHAR(36) NULL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `GlobetrotterV1`.`registered_user_view` (`id` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `GlobetrotterV1`.`saved_trip_view`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `GlobetrotterV1`.`saved_trip_view` (`id` INT);
...
BEGIN
	...
	SElECT `user` INTO v_user_id FROM registered_user WHERE `user` = p_user;
    SElECT location INTO v_location FROM home_location WHERE `user` = p_user;
	IF ISNULL(v_user_id) OR ISNULL(v_location) THEN
		SIGNAL SQLSTATE '45000'
	...
	ELSE
		UPDATE registered_user 
			SET username = p_username, 
				...
				birthday = p_birthday,
                gender = p_gender,
                primary_phone_country_code = p_primary_phone_country_code,
				primary_phone_number = p_primary_phone_number, 
				secondary_phone_country_code = p_secondary_phone_country_code,
        ...
        UPDATE location 
			...
			WHERE location.location_id = v_location;
    END IF;
...
-- -----------------------------------------------------
-- procedure usp_update_flight_activity
-- -----------------------------------------------------

USE `GlobetrotterV1`;
DROP procedure IF EXISTS `GlobetrotterV1`.`usp_update_flight_activity`;

DELIMITER $$
USE `GlobetrotterV1`$$
CREATE PROCEDURE `usp_update_flight_activity`(
	IN p_activity_id VARCHAR(36),
	IN p_start_time TIMESTAMP,
	IN p_end_time TIMESTAMP,
	IN p_start_location VARCHAR(36),
...
BEGIN
	...
	UPDATE flight_activity
		SET flight_offer_json_data = p_flight_offer_json_data 
        WHERE activity = p_activity_id;
END$$
...
CREATE  OR REPLACE VIEW `activity_view` AS (
    ...
    FROM activity 
        ...
        LEFT JOIN location AS origin 
			ON origin.location_id = start_location 
		LEFT JOIN location AS destination 
...
USE `GlobetrotterV1`;
CREATE  OR REPLACE VIEW `registered_user_view` AS (
	SELECT registered_user.`user` AS user_id, email, first_name, last_name, gender, 
		username, password_hashed, birthday, 
        address_line_1, address_line_2, city, state, country, postal_code, 
        CONCAT(primary_phone_country_code,'', primary_phone_number) AS primary_phone_number,
        CONCAT(secondary_phone_country_code,'', secondary_phone_number) AS secondary_phone_number
	FROM registered_user 
    INNER JOIN `user`
		ON `user`.user_id = registered_user.`user`
    INNER JOIN home_location 
		ON home_location.`user` = registered_user.`user`
	INNER JOIN location
		ON home_location.location = location.location_id
);
...
-- -----------------------------------------------------
-- View `GlobetrotterV1`.`saved_trip_view`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `GlobetrotterV1`.`saved_trip_view`;
DROP VIEW IF EXISTS `GlobetrotterV1`.`saved_trip_view` ;
USE `GlobetrotterV1`;
CREATE  OR REPLACE VIEW `saved_trip_view` AS (
	SELECT trip.`owner`, trip.trip_id, trip_name, 
		DATE_FORMAT(min(TIMESTAMP(departure_date, departure_time)), '%Y-%m-%d %H:%i:%s') AS trip_start, 
        DATE_FORMAT(max(TIMESTAMP(arrival_date, arrival_time)), '%Y-%m-%d %H:%i:%s') AS trip_end 
	FROM trip 
    LEFT JOIN flight_activity_view AS f 
		ON trip.trip_id = f.trip_id
