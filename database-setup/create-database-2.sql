...
CREATE  OR REPLACE VIEW `activity_view` AS (
    ...
    SELECT trip_id, trip_name, username, activity_id, start_time, end_time, src.location_name AS `source`, dst.location_name AS `dest` FROM activity
		INNER JOIN trip ON activity.trip = trip.trip_id
		INNER JOIN registered_user
			ON trip.`owner` = registered_user.`user`
        INNER JOIN location as src
			ON src.location_id = start_location
		INNER JOIN location as dst
			ON dst.location_id = end_location
		ORDER BY start_time
);
