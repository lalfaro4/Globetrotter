...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `GlobetrotterV1`.`registered_user_view` (`first_name` INT, `last_name` INT, `gender` INT, `username` INT, `birthday` INT, `address_line_1` INT, `address_line_2` INT, `city` INT, `state` INT, `country` INT, `postal_code` INT, `CONCAT(primary_phone_country_code,'',primary_phone_number)` INT);
...
USE `GlobetrotterV1`;
CREATE  OR REPLACE VIEW `registered_user_view` AS select first_name, last_name, gender, username, birthday, address_line_1, address_line_2, city, state, country, postal_code, CONCAT(primary_phone_country_code,'',primary_phone_number) from registered_user inner join home_location on home_location.user = registered_user.user inner join location on home_location.location = location.location_id;
