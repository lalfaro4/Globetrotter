...
<<<<<<< HEAD:database-setup/create-database.sql
-- Tue Jul 27 17:08:40 2021
=======
-- Tue Jul 27 03:21:22 2021
>>>>>>> 677221e08e3c0c36a939bf10b73fda6b2c8a4f97:database-setup/sql-scripts/create-database.sql
...
CREATE TABLE IF NOT EXISTS `GlobetrotterV1`.`service_provider` (
  ...
  UNIQUE INDEX `service_provider_id_UNIQUE` (`service_provider_id` ASC) VISIBLE,
  UNIQUE INDEX `service_provider_name_UNIQUE` (`service_provider_name` ASC) VISIBLE,
...
CREATE  OR REPLACE VIEW `registered_user_view` AS (
	SELECT registered_user.`user` AS user_id, 
		`user`.email, first_name, last_name, 
        gender, username, password_hashed, birthday, 
        address_line_1, address_line_2, city, 
        state, country, postal_code, 
        CONCAT(primary_phone_country_code,'',primary_phone_number) AS primary_phone_number 
	FROM registered_user 
    INNER JOIN `user` 
		ON `user`.user_id = user_id
